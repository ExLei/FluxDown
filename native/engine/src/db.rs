...
/// `tasks` 表 SELECT 列清单——用于 `task_from_row` 反序列化。
/// 注意：`file_present_at` 有意**不在此列**中。该列仅消费于
/// `MISSING_CLEANUP_WHERE` 谓词（SQL WHERE 子句过滤）和直接 DML
/// （`update_task_status` / `update_tasks_status_batch`），从不经
/// `TaskInfo` 读取。若未来需要将 `file_present_at` 读入 `TaskInfo`
/// 字段，必须同步补全三处：①本常量 ②`task_from_row` ③`TaskInfo` 结构体。
...
impl Db {
    ...
    ///
    /// 测试用 [`Db::is_missing_cleanup_candidate`] 复用同一谓词常量
    /// `MISSING_CLEANUP_WHERE`，确保测试覆盖率与实际查询语义一致。
    ...
    /// `#[cfg(test)]` ——生产执行路径由 `execute_missing_cleanup` 内
    /// `load_task_by_id` + 本地判定完成，无需此额外 DB 往返。
    #[cfg(test)]
