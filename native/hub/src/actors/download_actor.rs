...
pub async fn run(db_dir: PathBuf) {
    ...
    let cleanup_aux_tx = aux_tx.clone();
    tokio::spawn(async move {
        loop {
            cleanup_scan_done.notified().await;
            if cleanup_aux_tx.send(AuxSignal::CleanupTick).is_err() {
                break; // actor 已退出
            }
        }
    });
        ...
        tokio::select! {
            ...
            Some(aux) = aux_rx.recv() => {
                ...
                AuxSignal::Cleanup(cleanup_signal) => match cleanup_signal {
                    CleanupSignal::RequestCandidates(_) => {
                        let candidates: Vec<crate::signals::TaskInfo> = engine
                            .manager
                            ...
                            .map(|t| t.into())
                            .collect();
                        MissingCleanupCandidatesResult { candidates }.send_signal_to_dart();
                    }
                    CleanupSignal::Execute(msg) => {
                        let deleted = match futures_util::FutureExt::catch_unwind(
                            ...
                            Err(e) => {
                                ...
                                log_info!("[missing-cleanup] execute panicked: {}", msg);
                                0
                            }
                        };
                        if deleted > 0 { engine.manager.load_and_send_all_tasks().await; }
                        MissingCleanupExecuted { deleted: i32::try_from(deleted).unwrap_or(i32::MAX) }.send_signal_to_dart();
                    }
                },
                AuxSignal::CleanupTick => {
                    let deleted = engine.manager.auto_cleanup_missing_files().await;
                    if deleted > 0 { engine.manager.load_and_send_all_tasks().await; }
                }
